...
public final class ListenerRegister {
    ...
    private static final EventExecutor EXECUTOR = (listener, event) -> {
        ...
        final EventListener eventListener = (EventListener)listener;
        if (eventListener.eventClass() == null) { // Only for unique events. Not for EntityDamageByEntity and others
            eventListener.handle(event);
            return;
        }
        if (eventListener.eventClass().isAssignableFrom(event.getClass())) {
            eventListener.handle(event);
        }
    };
